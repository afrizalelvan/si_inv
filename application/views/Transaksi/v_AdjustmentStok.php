          ...
          <table width="100%">
            <tr>
              <td width="40%">
                <table width="100%">
                    <tr>
                      <td width="20%">ID Koreksi</td>
                      <td width="2%">:</td>
                      <td width="20%" id="txt-id_koreksi">
                      </td>
                    </tr>
                    <tr>
                      <td width="20%">Tanggal</td>
...
/* $('.tambah_data').click(function() {
  ...
  function load_data() {
    ...
    tabel = $('#datatable').DataTable({
      ...
      "paging": true,
      "ajax": {
        "url": '<?php echo base_url(); ?>Transaksi/load_data/Adjustment',
        "type": "POST",
        data  : ({tgl1,tgl2}), 
      },
      responsive: true,
  ...
  function tampil_edit(id,act){
    ...
    status = "update";
         ...
         $.ajax({
              ...
              type: 'POST',
              data: {id,jenis : "tr_koreksi_detail",field:'id_koreksi'},
              dataType: "JSON",
          })
          .done(function(data) {
            x= 0 ;
              id_koreksi = data[0].id_koreksi;

              $("#txt-id_koreksi").html(data[0].id_koreksi);
              
              $("#tanggal").html(data[0].add_time);
              $("#txt_tipe").html(data[0].tipe);
  ...
  function Batal(id,tipe){
    toastr.clear()
     ...
     $.ajax({
          url: '<?php echo base_url('Transaksi/batal_adjs'); ?>',
          type: 'POST',
          data: {id,tipe},
          dataType: "JSON",
  ...
  function printContent(){
    link = "<?=base_url('Transaksi/print_relokasi')?>?id="+id_koreksi;
